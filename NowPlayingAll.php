...
$result = mysqli_query($db, 'SELECT movie_name, rating, movieLength FROM movies;');
...
if (mysqli_num_rows($result) > 0) {
    ...
    echo "<table>";
    echo "<thead><tr><th>Movie Title</th><th>Rating</th><th>Movie Length</th></tr></thead>";
    echo "<tbody>";
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        foreach ($rows as $row) {
            echo "<tr style='border: 1px solid black;'><td>{$row['movie_name']}</td><td>{$row['rating']}</td><td>{$row['movieLength']}</td></tr>";
        }
        echo "</tbody>";
        echo "</table>";
    }
